...
= Branch vs Branchless Multiplication Performance
:toc:
:icons: font

== Overview

On modern CPUs, the branchless multiplication approach (using `value * sign`) is typically *faster* in most scenarios.
...
* An `if` statement creates a branch that must be predicted
* Misprediction cost: ~10-20 cycles (pipeline flush)
* Correct prediction cost: ~1-2 cycles

=== 2. Multiplication Speed
...
[cols="1,3"]
|===
|Approach |Optimal Scenario

|Multiplication
|When signs are unpredictable (random data)

|Branch
|When signs are highly predictable (>95% same direction)
|===

== Conclusion

TIP: For general-purpose code with mixed/unpredictable data as in our puzzle, multiplication is faster because avoiding
branch mispredictions saves more time than the tiny extra cost of multiplication.
