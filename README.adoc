...
== Day 3: Bounds Check Elimination

The puzzle requires finding the two largest digits on each line, where the left digit `l` must leave at least one position for the right digit `r`.

=== The Problem with Bounds Checks
